...
module Fresh = struct
  ...
  module Make () = struct
    ...
    let tty_of_etype (e : Ty.t) : DTerm.ty =
      ...
      | Ty_fp 32 -> float32_ty
      | Ty_fp 64 -> float64_ty
      | Ty_fp _ | Ty_bitv _ | Ty_list | Ty_app | Ty_unit | Ty_none ->
        assert false
    ...
    module Bv = struct
      ...
      let encode_cvtop sz op e =
        let op' =
          ...
          | TruncSF32 | TruncSF64 ->
            DTerm.Float.to_sbv sz DTerm.Float.roundTowardZero
          | TruncUF32 | TruncUF64 ->
            DTerm.Float.to_ubv sz DTerm.Float.roundTowardZero
          | Reinterpret_float when sz = 32 -> DTerm.Float.ieee_format_to_fp 8 24
          | Reinterpret_float when sz = 64 ->
            DTerm.Float.ieee_format_to_fp 11 53
    ...
    let encode_unop = function
      ...
      | (Ty.Ty_list | Ty_app | Ty_unit | Ty_none) as op ->
        Fmt.failwith
        ...
        | Some a ->
          Some
            (Value.Num
               ...
               | _ -> assert false ) ) )
      | Ty_str | Ty_list | Ty_app | Ty_unit | Ty_none ->
        Fmt.failwith
