...
module Make (M_with_make : M_with_make) : S_with_fresh = struct
  module Make_ (M : M) : S = struct
    ...
    let str2int = M.Func.make "string_to_int" [ sort Ty_str ] (sort Ty_int)

    let real2str = M.Func.make "real_to_string " [ sort Ty_real ] (sort Ty_str)

    let str2real = M.Func.make "string_to_real" [ sort Ty_str ] (sort Ty_real)

    let str_trim = M.Func.make "string_trim" [ sort Ty_str ] (sort Ty_str)

    let f32_to_i32 =
      M.Func.make "f32_to_i32" [ sort (Ty_fp 32) ] (sort (Ty_bitv 32))

    let f64_to_i64 =
      M.Func.make "f64_to_i64" [ sort (Ty_fp 64) ] (sort (Ty_bitv 64))
