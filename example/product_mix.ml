...
let int x = Expr.(Val (Int x) @: Ty_int)
let symbol x = Expr.(mk_symbol Symbol.(x @: Ty_int))
let ( = ) i1 i2 = Expr.(Relop (Eq, i1, i2) @: Ty_int)
let ( >= ) i1 i2 = Expr.(Relop (Ge, i1, i2) @: Ty_int)
let ( <= ) i1 i2 = Expr.(Relop (Le, i1, i2) @: Ty_int)
let ( + ) i1 i2 = Expr.(Binop (Add, i1, i2) @: Ty_int)
let ( * ) i1 i2 = Expr.(Binop (Mul, i1, i2) @: Ty_int)
let ( && ) b1 b2 = Expr.(Binop (And, b1, b2) @: Ty_bool)
