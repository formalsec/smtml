...
module Infix = struct
  ...
  let int8 x = value (Bitv (Bitvector.make (Z.of_int x) 8))

  let int32 x = value (Bitv (Bitvector.make (Z.of_int32 x) 32))

  let int64 x = value (Bitv (Bitvector.make (Z.of_int64 x) 64))
