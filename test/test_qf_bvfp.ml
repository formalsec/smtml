...
let () =
  let solver = Z3.create ~logic:QF_BVFP () in
  let x = F32.sym "x" in
  Z3.add solver F32.[ x = v 0.504 ];
  assert (Z3.check solver F32.[ v 1. = Unop (Ceil, x) @: Ty_fp S32 ]);
  assert (Z3.check solver F32.[ v 0. = Unop (Floor, x) @: Ty_fp S32 ]);
  assert (Z3.check solver F32.[ v 0. = Unop (Trunc, x) @: Ty_fp S32 ]);
  let y = F32.sym "y" in
  Z3.add solver F32.[ y = v (-2.7) ];
  assert (Z3.check solver F32.[ v (-2.) = Unop (Ceil, y) @: Ty_fp S32 ]);
  assert (Z3.check solver F32.[ v (-3.) = Unop (Floor, y) @: Ty_fp S32 ]);
  assert (Z3.check solver F32.[ v (-2.) = Unop (Trunc, y) @: Ty_fp S32 ])
