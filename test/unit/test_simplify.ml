...
let () =
  assert (
    let unary = mk @@ Unop (Ty_bitv S32, Neg, I32.v 1l) in
    simplify unary = I32.v (Int32.neg 1l) );
  assert (
    let binary = mk @@ Binop (Ty_bitv S32, Add, I32.v 1l, I32.v 1l) in
    simplify binary = I32.v 2l );
  assert (
    let triop = mk @@ Triop (Ty_bool, Ite, Bool.v true, I32.v 1l, I32.v 0l) in
    simplify triop = I32.v 1l );
  assert (
    let relop = mk @@ Relop (Ty_bitv S32, Lt, I32.v 2l, I32.v 1l) in
    simplify relop = mk @@ Val False );
  assert (
    let cvtop = mk @@ Cvtop (Ty_bitv S32, WrapI64, I64.v 1L) in
